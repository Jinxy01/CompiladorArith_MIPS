...
let () =
  ...
  if !ifile="" then begin eprintf "Nenhum arquivo a compilar\n@?"; exit 1 end;
  ...
  try
    (* Parsing: A função Parser.prog transforma o buffer de análise léxica  
       numa árvore de sintaxe abstracta se nenhum erro  (léxico ou sintáctico) 
       foi detectado.
  ...
  with
	...
	(* Erro sintáctio. Recupera-se a posição e converte-se para número 
           de linha *)
	localisation (Lexing.lexeme_start_p buf);
	eprintf "Erro durante a análise sintáctica: %s@." (Lexing.lexeme buf);
	exit 1
    | Tipagem.VarUndef s->
	...
	(* Erro de utilização de variáveis durante a compilação *)
	eprintf
	  "Erro de tipagem: estava a espera de %s e recebi %s@." e r;
	exit 1
	| Compile.OutOfBounds s ->
	(* Erro de utilização de variáveis durante a compilação *)
	eprintf
